...
private:
	...
	inline bool determineWhetherParticleIsInDomain(const F64vec & pos, const F64ort & domain) {
		bool ret = true;
	...
	S32 searchWhichDomainParticleGoTo(const F64vec & pos, const S32 n_domain[], const F64ort domain[]) {
...
public:
	...
	void readParticleImpl(const char * const filename, const char * const format, Theader * const header, void (Tptcl::*pFuncPtcl)(FILE*), S32 (Theader::*pFuncHead)(FILE*), const char * open_format) {
	...
	template<class Theader>
	void writeParticleImpl(const char * const filename, const char * const format, const Theader * const header, void (Tptcl::*pFuncPtcl)(FILE*) const, void (Theader::*pFuncHead)(FILE*) const, const char * open_format) {
		if (format == NULL) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
	...
	template<class Theader>
	void writeParticle_VTK_Ascii_Impl(const char * const filename_vtk,
		...
		MPI::COMM_WORLD.Gatherv(ptcl_.getPointer(), n_ptcl_, GetDataType<Tptcl>(), ptcl, n_ptcl, n_ptcl_displs, GetDataType<Tptcl>(), 0);
		if(Comm::getRank() == 0) {
			FILE* fp_vtk = fopen(filename_vtk, "w");
			if(fp_vtk== NULL) {
				PARTICLE_SIMULATOR_PRINT_ERROR("can not open output vtk file");
				std::cerr<<"output file: "<<filename_vtk<<std::endl;
				Abort(-1);
			}
			//TODO we need preheader because VTK format need only one line of declaration
			header->writeVTKAscii(fp_vtk);
			for (S32 i = 0; i < n_tot; ++i) {
				ptcl[i].writeAscii_pos(fp_vtk);
			}
			header->writeAscii_scalars_pre_header(fp_vtk);
			//TODO only consider one kind particle
			for (S32 s = 0;
					s < sizeof(ptcl[0].scalars) / sizeof((ptcl[0].scalars[0]));
					++s) {
				header->writeAscii_scalars_header(ptcl[0].scalars[s], fp_vtk);
				for (S32 i = 0; i < n_tot; ++i) {
					ptcl[i].writeAscii_scalars(ptcl[0].scalars[s], fp_vtk);
				}
			}
			fclose(fp_vtk);
		}
		...
		const S32 n_tot = ptcl_.size();
		if (Comm::getRank() == 0) {
//			FILE* fp_ascii = fopen(filename_ascii, "w");
//			if(fp_ascii == NULL) {
//				PARTICLE_SIMULATOR_PRINT_ERROR("can not open output vtk file");
//				std::cerr<<"output file: "<<filename_ascii<<std::endl;
//				Abort(-1);
//			}
//
//			header->writeAsciiTxt(fp_ascii);
//			for (S32 i = 0; i < n_tot; ++i) {
//				ptcl_[i].writeAsciiTxt(fp_ascii);
//			}
//			fclose(fp_ascii);
			FILE* fp_vtk = fopen(filename_vtk, "w");
			//TODO we need preheader because VTK format need only one line of declaration
			header->writeVTKAscii(fp_vtk);
			for (S32 i = 0; i < n_tot; ++i) {
				ptcl_[i].writeAscii_pos(fp_vtk);
			}
			header->writeAscii_scalars_pre_header(fp_vtk);
			//TODO only consider one kind particle
			double b = sizeof(ptcl_[0].scalars) / sizeof((ptcl_[0].scalars[0]));
			for (S32 s = 0;
					s < sizeof(ptcl_[0].scalars) / sizeof((ptcl_[0].scalars[0]));
					++s) {
				header->writeAscii_scalars_header(ptcl_[0].scalars[s], fp_vtk);
				for (S32 i = 0; i < n_tot; ++i) {
					ptcl_[i].writeAscii_scalars(ptcl_[0].scalars[s], fp_vtk);
				}

			}
			for (S32 s = 0;
					s < sizeof(ptcl_[0].vectors) / sizeof((ptcl_[0].vectors[0]));
					++s) {
				header->writeAscii_vectors_header(ptcl_[0].vectors[s], fp_vtk);
				for (S32 i = 0; i < n_tot; ++i) {
					ptcl_[i].writeAscii_vectors(ptcl_[0].vectors[s], fp_vtk);
				}
			}
			fclose(fp_vtk);
		}
	...
	template<class Theader>
	void writeParticle_VTK_Ascii(const char * const filename_vtk,
			const Theader& header) {
		writeParticle_VTK_Ascii_Impl(filename_vtk, &header);
	}
#endif
	//write
	template<class Theader>
	...
	Tptcl & getParticle(const S32 id = 0) {
		return ptcl_[id];
	}
	const Tptcl & getParticle(const S32 id = 0) const {
		return ptcl_[id];
	}
	Tptcl * getParticlePointer(const S32 id = 0) const {
		return ptcl_ + id;
	}
...
// *******************************************************************        
	...
	inline S32 getUniformDistributionFromArg1ToArg2(S32 arg1, S32 arg2) {
		S32 random_number;
	 ...
	 */
	void getSampleParticle(S32 & number_of_sample_particle, F64vec pos_sample[], const F32 weight) {
...
// modified to limit # of sample particles by M.I. 
	...
	bool checkExchangeParticleSumOfNumberOfParticle(DomainInfo & dinfo, S32 ntot_init);
...
#pragma omp parallel for
	 ...
	 */
	CountT getNumberOfParticleSendLocal() const {
		return (CountT) n_ptcl_send_;
	}
	CountT getNumberOfParticleRecvLocal() const {
		return (CountT) n_ptcl_recv_;
	}
	CountT getNumberOfParticleSendGlobal() const {
		return Comm::getSum((CountT) n_ptcl_send_);
	}
	CountT getNumberOfParticleRecvGlobal() const {
		return Comm::getSum((CountT) n_ptcl_recv_);
	}
	...
	void dumpMemSizeUsed(std::ostream & fout) {
		F64 sum_loc, sum_max;
		sum_loc = (double) (ptcl_.getMemSize() + idx_remove_ptcl_.getMemSize() + ptcl_send_.getMemSize() + ptcl_recv_.getMemSize()) * 1e-9;
		if (Comm::getRank() == 0) {
