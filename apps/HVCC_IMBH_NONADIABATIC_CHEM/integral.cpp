...
void InitialKick(PS::ParticleSystem<RealPtcl>& sph_system, const PS::F64 dt) {
	for (PS::S32 i = 0; i < sph_system.getNumberOfParticleLocal(); ++i) {
		...
		if (sph_system[i].eng_half < 0.0) {
			std::cout << sph_system[i].eng_half << "in InitialKick "
					<< std::endl;
		}
...
//	PS::F64 energy_dot = hydro.eng_dot * hydro.dens * PARAM::SM / (PARAM::SL * PARAM::ST * PARAM::ST * PARAM::ST);
	...
	PS::F64 abs_tol_eng, rel_tol_eng = fac, rel_diff_eng;
	double loop = 1000;
	double T = 0.0;
...
//		std::cout << dt << " " << time_passed << " " << tleft << "out loop" << std::endl;
		...
		for (PS::U64 sub_step = 0; sub_step < sub_try_steps; sub_step++) {
...
//			Lambda = 1.4e-22*NUMDENS_CGS*NUMDENS_CGS*pow(T/(3.e4),beta);
			...
			K1 = RATE_CR_HII_HeII(abundance_e);
			K2 = RATE_XR_HII_HeII(T, abundance_e, abundance_HI,
					PARAM::col_dens_H2);
			K3 = RATE_H_col_HII(T);
			K4 = RATE_HIIe_HPhoton(T);
			K5 = RATE_HHGrain_H2(T, tdust);
			K6 = RATE_HHm_H2_e(T, abundance_HI, abundance_HII);
			K7 = RATE_H2_UV_2H(T, PARAM::col_dens_H2, mu, abundance_H2);
			K8 = RATE_HH2_3H(T);
			K9 = RATE_H2_CR_2H(abundance_e);

			C_HII = K1 * abundance_HI * NUMDENS_CGS
					+ K2 * abundance_HI * NUMDENS_CGS
					+ K3 * NUMDENS_CGS * abundance_e * NUMDENS_CGS
							* abundance_HI;
			D_HII = K4 * abundance_e * NUMDENS_CGS;
			C_HI = K4 * abundance_e * abundance_HII * NUMDENS_CGS * NUMDENS_CGS
					+ K7 * abundance_H2 * NUMDENS_CGS
					+ K8 * abundance_H2 * abundance_HI * NUMDENS_CGS
					+ K9 * abundance_H2 * NUMDENS_CGS;
			D_HI = K1 + K2 + K3 * abundance_e * NUMDENS_CGS
					+ K5 * abundance_HI * NUMDENS_CGS;
			C_H2 = K5 * abundance_HI * abundance_HI * NUMDENS_CGS * NUMDENS_CGS
					+ K6 * abundance_e * abundance_HI * NUMDENS_CGS
							* abundance_HI * NUMDENS_CGS;
			D_H2 = K7 + K8 * NUMDENS_CGS * abundance_HI + K9;
			C_CO = _C_CO(NUMDENS_CGS, abundance_OI, abundance_CII,
					abundance_H2);
			D_CO = _D_CO();
			abundance_HI = C_HI / (NUMDENS_CGS) / D_HI
					+ (abundance_HI_init - C_HI / (NUMDENS_CGS) / D_HI)
			...
			abundance_H2 = abundance_H2
					/ (abundance_HII + abundance_HI + 2. * abundance_H2);
			abundance_CII = fmax(abundance_CI - abundance_CO, 0.0);
			mu = abundance_HI + abundance_HII + 2.0 * abundance_H2
					+ 4.0 * abundance_HeI;
			mu /= (abundance_HI + abundance_HII + abundance_H2 + abundance_HeI
					+ abundance_e);
			mu = 2.34;
...
//                      std::cout<<time_passed <<" "<<oneDynTimeStep * PARAM::ST<<" "<<energy <<std::endl;
			if (rel_diff_eng + 1.e-6 < rel_tol_eng) {
...
//			if (fabs(T - temprature_init) < 1.e-12) {
//
//				hydro.Lambda = Lambda * 1e26;
//				hydro.Gamma = Gamma * 1e26;
//				hydro.cooling_timescale = ((energy / fabs(Lambda))
//						* MASSDENS_CGS) / PARAM::yr;
