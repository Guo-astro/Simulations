...
void SetupIC_HVCC_RESTART_file(PS::ParticleSystem<RealPtcl>& sph_system, PS::F64 *end_time) {
	...
	*end_time = 1000000;
	...
	FileHeader header;
#ifdef RESTART
	std::cout << PARAM::SVel/1e5	<< std::endl;


	sph_system.readParticleAscii("result/0465.dat", header);
	std::cout << "setup...ptcl num " << sph_system.getNumberOfParticleLocal()	<< std::endl;
