#include "../CloudCloudCollision_Plane_Plane/fittings.h"
...
void FinalKick(PS::ParticleSystem<RealPtcl>& sph_system, const PS::F64 dt,
	...
	for (PS::S32 i = 0; i < sph_system.getNumberOfParticleLocal(); ++i) {
//		sph_system[i].smth_min =std::min(smth_min, sph_system[i].smth);
//		sph_system[i].smth_max =std::max(smth_max, sph_system[i].smth);
