...
//		}
//		if(grav1 != 0.0){
//			if(fabs(grav1-grav2)<1e-4){
//				std::cout<<grav1<<std::endl;
//			}
//		}
		PS::F64 ppre, p, v;
		PS::F64 W1, W2;
		...
		double grav1 = eij * ep_i.grav;
		double grav2 = eij * ep_j.grav;
		//		if(grav1 != 0.0){
		//			if(fabs(grav1-grav2)<1e-4){
		//				std::cout<<grav1<<std::endl;
		//			}
...
//		}
		if (p1 <= 1.0e-6 && p2 <= 1.0e-6) {
			...
			double w_max = fmax(W1, W2);
			double w_min = fmin(W1, W2);
			const PS::F64 hmax = fmax(ep_i.smth, ep_j.smth);
			const PS::F64 hmin = fmin(ep_i.smth, ep_j.smth);
//			if ( delta>hmin) {
//				vstar = .5 * (v1 + v2);
//			}
		}
	...
	void operator ()(const EPI::Hydro* const ep_i, const PS::S32 Nip, const EPJ::Hydro* const ep_j, const PS::S32 Njp, RESULT::Hydro* const hydro) {
		...
		for (PS::S32 i = 0; i < Nip; ++i) {
			hydro[i].clear();
			PS::F64 smth_min = 1.0e+30; //set VERY LARGE VALUE
			PS::F64 smth_max = -1.0e+30; //set VERY LARGE VALUE
			for (PS::S32 j = 0; j < Njp; ++j) {

				smth_min = std::min(smth_min, ep_j->smth);
				smth_max = std::max(smth_max, ep_j->smth);
			}
			...
			for (PS::S32 j = 0; j < Njp; ++j) {
				...
				const PS::F64 hmax = fmax(ep_i->smth, ep_j->smth);
				const PS::F64 hmin = fmin(ep_i->smth, ep_j->smth);
				const PS::F64 hratio = hmax / hmin;
				const PS::F64 cmax = fmax(ep_i->snds, ep_j->snds);
				const PS::F64 cmin = fmin(ep_i->snds, ep_j->snds);

				PS::F64vec fac_acc = 0.0;
				PS::F64 fac_eng_dot = 0.0;
//				velref = (jth.vel - ith.vel) * ndir;

				calc_riemann_solver(ith, jth, sstar, delta, ndir, ith.dt, PSTAR, VSTAR);
				fac_acc = -PSTAR * F_ij * ndir;
//				if (ith.smth_max != 0.0) {
//					std::cout << ith.smth_max << " " << ith.smth_min << std::endl;
//				}
				PS::F64 smth_mean = .5 * (smth_max + smth_min);
//				if (delta< hmin) {

				fac_eng_dot = -PSTAR * (VSTAR - ith.vel_half * ndir) * F_ij;
//				} else {
//					fac_eng_dot = -PSTAR * .5*(jth.vel - ith.vel) * ndir * F_ij;
//
//				}
...
//			double current_phi = getPhi(_r) > 0 ? getPhi(_r) : 1e-4;
//			double current_phi_dash = getPhi_dash(_r);
//			PS::F64vec acc_factor = ep_i[i].pos / _r;
//
//			hydro[i].acc = 1.5 * sqrt(current_phi) * current_phi_dash * acc_factor;
			hydro[i].dt =fmin( 0.1 * ith.smth / ith.snds, 0.1 * ith.smth / sqrt(ith.vel*ith.vel));
			...
			for (PS::S32 j = 0; j < Njp; ++j) {
				...
				if (ith.pos != jth.pos) {
					...
					if (typeid(TParticleJ) == typeid(EPJ::Grav)) {
						...
						const PS::F64 m_dr3_inv = jth.mass * math::pow3(dr_inv + 0.0000000001);
						PS::F64 hmax = fmax(ith.smth, jth.smth);
						PS::F64 hmin = fmin(ith.smth, jth.smth);
						PS::F64 x = hmin / hmax;
						PS::F64 fit = 1.0 / sqrt(1.0 + x * x);
