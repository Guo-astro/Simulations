...
//Give necessary values to FDPS
	...
	void readAscii(FILE* fp) {
		...
		PS::S64 minus_eng;
		PS::F64 radialForce_extF_mag, div_B, deltaB;

		fscanf(fp, "%lld\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf"
				"\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf"
				"\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf"
				"\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf", &this->id,
				&this->mass, &this->smth, &this->pos.x, &this->pos.y,
				...
				&radialForce_mag, &radialForce_grav_mag, &radialForce_extF_mag,
				&tem, &NUMDENS_CGS, &this->Lambda, &this->cooling_timescale,
				&this->sg_timescale, &this->bh_timescale,
				&this->hydro_timescale, &this->eng_timescale,
				&this->abundances[0], &this->abundances[1],
				&this->abundances[2], &this->abundances[3],
				&this->abundances[4], &this->abundances[5],
				&this->abundances[6], &this->abundances[7],
				&this->abundances[8], &this->abundances[9], &mu, &div_B,
				&deltaB);
//		eng = this->pres / (this->dens * (PARAM::GAMMA - 1));
