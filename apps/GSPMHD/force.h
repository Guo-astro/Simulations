...
//				const PS::F64 F_ij = (kernel.gradW_scoord(s_ij, sqrt(2.0) * ith.smth)  *VIJ_I + kernel.gradW_scoord(s_ij, sqrt(2.0) * jth.smth)  *VIJ_J);
			...
			const PS::F64 C_fast = sqrt(
					(Gamma * ith.pres + ith.MagneticB * ith.MagneticB)
							/ ith.dens);
			PS::F64 dt = 0.3 * PARAM::C_CFL * ith.smth / C_fast;
			hydro[i].dt = dt;
...
//
		...
		const PS::F64 PTLRP = orig_PTL + fac_BperpL2RP+fac_PresLRP;
		//
		//		//////R -> i///////////////////////////
		//
		...
		const PS::F64 PTRRP = orig_PTR -  fac_BperpR2RP-fac_PresRRP;
		...
		if (ptRRP < 0.0 || ptLRP < 0.0 || densLRP < 0.0 || densRRP < 0.0
			...
			PS::F64 PTRRP_star = PTRRP;
			PS::F64 PTLRP_star = PTLRP;
			
			...
			while (loop) {
				loop += 1;
				ptStar_old = ptStar;
				PS::F64 DR2 = pow((gamma + 1) * ptRRP + (gamma - 1) * ptStar,
						2.)
						- (gamma - 2) * BperpR2RP
								* (2. * (gamma - 3) * ptRRP
										+ 2. * (gamma + 3) * ptStar
										- (gamma - 2) * BperpR2RP);
				PS::F64 DL2 = pow((gamma + 1) * ptLRP + (gamma - 1) * ptStar,
						2.)
						- (gamma - 2) * BperpL2RP
								* (2. * (gamma - 3) * ptLRP
										+ 2. * (gamma + 3) * ptStar
...
//				std::cout << loop << " otStar:" << ptStar << "otStar:" << ptRRP << " " << M_R2 << " " << ptLRP << " " << M_L2 << " " << sqrt(DL2) << " " << (gamma - 3.0) * ptLRP + (gamma + 3) * ptStar - (gamma - 2) * BperpL2RP << std::endl;
				...
				if (1.e-12 + fabs((ptStar - ptStar_old) / ptStar) < 0.01) {
		...
		if (sgn(_Bpara) > 0) {
			...
			const PS::F64vec orig_vperpPlusL = ep_L.vel
					- (ep_L.vel * ndir) * ndir;
			const PS::F64vec orig_vperpMinusR = ep_R.vel
					- (ep_R.vel * ndir) * ndir;
			const PS::F64vec orig_BperpPlusL = ep_L.MagneticB
					- (ep_L.MagneticB * ndir) * ndir;
			const PS::F64vec orig_BperpMinusR = ep_R.MagneticB
					- (ep_R.MagneticB * ndir) * ndir;
//////////////////////////////////////////////////////
			const PS::F64vec orig_vperpPlusR = ep_R.vel
					- (ep_R.vel * ndir) * ndir;
			const PS::F64vec orig_vperpMinusL = ep_L.vel
					- (ep_L.vel * ndir) * ndir;
			const PS::F64vec orig_BperpPlusR = ep_R.MagneticB
					- (ep_R.MagneticB * ndir) * ndir;
			const PS::F64vec orig_BperpMinusL = ep_L.MagneticB
					- (ep_L.MagneticB * ndir) * ndir;
			//////////////////////////////////////////////////////
			//////////////////////////////////////////////////////
			//////////////////////////////////////////////////////
			...
			const PS::F64vec orig_JPlusL = orig_vperpPlusL
					- orig_BperpPlusL * orig_idensLsqrt;
			const PS::F64vec orig_JMinusR = orig_vperpMinusR
					+ orig_BperpMinusR * orig_idensRsqrt;

			const PS::F64vec orig_JPlusR = orig_vperpPlusR
					- orig_BperpPlusR * orig_idensRsqrt;
			const PS::F64vec orig_JMinusL = orig_vperpMinusL
					+ orig_BperpMinusL * orig_idensLsqrt;
			...
			const PS::F64vec DeltaJMinus_dashL = 2.0 * DeltaJMinusL
					- DiffJMinusL;
			const PS::F64vec DeltaJPlus_dashR = 2.0 * DeltaJPlusR - DiffJPlusR;
			const PS::F64vec DeltaJMinus_dashR = 2.0 * DeltaJMinusR
					- DiffJMinusR;
			...
			const PS::F64 vperpxLMoC = orig_vperpPlusL.x + fac_vperpxLMoC;
			const PS::F64 vperpyLMoC = orig_vperpPlusL.y + fac_vperpyLMoC;
			const PS::F64 vperpzLMoC = orig_vperpPlusL.z + fac_vperpzLMoC;
			const PS::F64 BperpxLMoC = orig_BperpPlusL.x + fac_BperpxLMoC;
			const PS::F64 BperpyLMoC = orig_BperpPlusL.y + fac_BperpyLMoC;
			const PS::F64 BperpzLMoC = orig_BperpPlusL.z + fac_BperpzLMoC;

			const PS::F64 vperpxRMoC = orig_vperpMinusR.x - fac_vperpxRMoC;
			const PS::F64 vperpyRMoC = orig_vperpMinusR.y - fac_vperpyRMoC;
			const PS::F64 vperpzRMoC = orig_vperpMinusR.z - fac_vperpzRMoC;
			const PS::F64 BperpxRMoC = orig_BperpMinusR.x - fac_BperpxRMoC;
			const PS::F64 BperpyRMoC = orig_BperpMinusR.y - fac_BperpyRMoC;
			const PS::F64 BperpzRMoC = orig_BperpMinusR.z - fac_BperpzRMoC;
			//////////////////////////////////////////////////////
			//////////////////////////////////////////////////////
			//////////////////////////////////////////////////////
			//////////////////////////////////////////////////////
			const PS::F64vec vperpPlusMoC(vperpxLMoC, vperpyLMoC, vperpzLMoC);
			const PS::F64vec BperpPlusMoC(BperpxLMoC, BperpyLMoC, BperpzLMoC);
			const PS::F64vec vperpMinusMoC(vperpxRMoC, vperpyRMoC, vperpzRMoC);
			const PS::F64vec BperpMinusMoC(BperpxRMoC, BperpyRMoC, BperpzRMoC);

			const PS::F64 idensMinusMoC = 1. / densRMoC;
			const PS::F64 densMinusMoCsqrt = sqrt(densRMoC);
			const PS::F64 idensMinusMoCsqrt = 1. / sqrt(densRMoC);
			const PS::F64 idensPlusMoC = 1. / densLMoC;
			const PS::F64 densPlusMoCsqrt = sqrt(densLMoC);
			const PS::F64 idensPlusMoCsqrt = 1. / sqrt(densLMoC);

			BperpMoC = (BperpMinusMoC * idensMinusMoCsqrt
					+ BperpPlusMoC * idensPlusMoCsqrt
					+ (-vperpPlusMoC + vperpMinusMoC))
					/ (idensMinusMoCsqrt + idensPlusMoCsqrt);
			VperpMoC = (densMinusMoCsqrt * vperpMinusMoC
					+ densPlusMoCsqrt * vperpPlusMoC
					+ (-BperpPlusMoC + BperpMinusMoC))
					/ (densMinusMoCsqrt + densPlusMoCsqrt);
		} else {
			const PS::F64vec orig_vperpPlusR = ep_R.vel
					- (ep_R.vel * ndir) * ndir;
			const PS::F64vec orig_vperpMinusL = ep_L.vel
			...
			//////////////////////////////////////////////////////
			//////////////////////////////////////////////////////
			const PS::F64vec vperpPlusMoC(vperpxRMoC, vperpyRMoC, vperpzRMoC);
			const PS::F64vec BperpPlusMoC(BperpxRMoC, BperpyRMoC, BperpzRMoC);
			const PS::F64vec vperpMinusMoC(vperpxLMoC, vperpyLMoC, vperpzLMoC);
			const PS::F64vec BperpMinusMoC(BperpxLMoC, BperpyLMoC, BperpzLMoC);

			const PS::F64 idensMinusMoC = 1. / densLMoC;
			const PS::F64 densMinusMoCsqrt = sqrt(densLMoC);
			const PS::F64 idensMinusMoCsqrt = 1. / sqrt(densLMoC);
			const PS::F64 idensPlusMoC = 1. / densRMoC;
			const PS::F64 densPlusMoCsqrt = sqrt(densRMoC);
			const PS::F64 idensPlusMoCsqrt = 1. / sqrt(densRMoC);
