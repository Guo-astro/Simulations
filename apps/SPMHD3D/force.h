	...
	void operator ()(const EPI::Hydro* const ep_i, const PS::S32 Nip, const EPJ::Hydro* const ep_j, const PS::S32 Njp, RESULT::Hydro* const hydro) {
		for (PS::S32 i = 0; i < Nip; ++i) {
			...
			for (PS::S32 j = 0; j < Njp; ++j) {
				...
				const PS::F64vec gradWi = kernel.gradW(dr, ith.smth);
				const PS::F64vec gradWj = kernel.gradW(dr, jth.smth);
				const PS::F64 magBi2 = ith.MagneticB * ith.MagneticB;
				...
				hydro[i].acc -= jth.mass * (ith.pres * gradWi / (ith.dens * ith.dens) + jth.pres * gradWj / (jth.dens * jth.dens) + AV * gradW);
				hydro[i].eng_dot += jth.mass * (ith.pres * gradWi * dv / (ith.dens * ith.dens) + 0.5 * AV * dv * gradW);
			}
