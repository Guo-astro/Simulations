	...
	void operator ()(const EPI::Hydro* const ep_i, const PS::S32 Nip, const EPJ::Hydro* const ep_j, const PS::S32 Njp, RESULT::Hydro* const hydro) {
		...
		for (PS::S32 i = 0; i < Nip; ++i) {
			...
			for (PS::S32 j = 0; j < Njp; ++j) {
				...
				PS::F64vec fac_acc = -PSTAR * F_ij * ndir;
				PS::F64 fac_eng_dot = -PSTAR * (VSTAR - ith.vel_half * ndir) * F_ij;
//				velref = (jth.vel - ith.vel) * ndir;
				if (delta < 3.0 * hmin) {

				} else {
					fac_acc = -.5 * (ith.pres + jth.pres) * F_ij * ndir;
					fac_eng_dot = -.5 * (ith.pres + jth.pres) * (jth.vel_half - ith.vel_half) * ndir * F_ij;
